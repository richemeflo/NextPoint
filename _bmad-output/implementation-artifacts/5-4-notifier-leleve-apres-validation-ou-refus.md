...
- [x] Verifier les preconditions et dependances de Story 5.4 (AC: tous)
...
- `approve_booking` cree une notification `booking_approved` pour l'eleve demandeur.
- `refuse_booking` cree une notification `booking_refused` avec commentaire coach visible dans le corps et les metadonnees.
- Les demandes auto-refusees lors d'une approbation concurrente creent aussi une notification et un historique.
- Les tentatives push `pending` sont envoyees via l'Edge Function Expo apres les mutations coach.
...
- 2026-06-30: Notifications eleve creees apres validation, refus et auto-refus de demande.
