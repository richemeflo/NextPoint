...
1. Given un élève connecté When il choisit une demande de cours collectif Then il peut sélectionner des joueurs de l’application And le demandeur est inclus dans la demande collective.
2. Given une demande collective valide When elle est envoyée Then une réservation ou demande collective pending est créée And la liste des participants est conservée.
3. Given une sélection de joueurs invalide ou non autorisée When l’élève tente d’envoyer la demande Then l’action est refusée avec une erreur claire And aucune donnée participant incohérente n’est créée.
4. Given une demande collective When le coach ouvre son détail Then il voit le demandeur et les participants sélectionnés And il peut traiter la demande comme les autres demandes pending.
5. Given le type collectif et la durée When le tarif est déterminé Then le tarif collectif applicable est sélectionné automatiquement And aucun tarif individuel incompatible n’est appliqué.

## Tasks / Subtasks

- [x] Verifier les preconditions et dependances de Story 4.7 (AC: tous)
...
Le fichier precedent le plus proche est `4-6-expirer-automatiquement-les-demandes-pending.md`; le lire avant implementation pour reprendre conventions, scripts, decisions et limites deja documentees.
...
- `_bmad-output/implementation-artifacts/4-7-demander-un-cours-collectif-avec-joueurs-selectionnes.md`
