...
begin
  ...
  set included_sessions = case
        when p_delta = 1 then included_sessions + 1
        else included_sessions
      end,
      used_sessions = case
        when p_delta = -1 then used_sessions + 1
        else used_sessions
      end,
      status = case
        when p_delta = -1
          and used_sessions + 1 = included_sessions
  ...
  where target_pack.id = p_pack_id
    ...
    and (
      (
        p_delta = -1
        and target_pack.status = 'active'
        and target_pack.used_sessions < target_pack.included_sessions
      )
      or (
        p_delta = 1
  ...
  values (
    ...
    case
      when p_delta = 1
        then 'lesson_pack_adjusted'::public.student_history_event_type
      else 'lesson_pack_consumed'::public.student_history_event_type
    end,
    case
    ...
    case
      ...
      else 'Session de pack consommée'
    end,
...
comment on function public.adjust_lesson_pack_sessions(uuid, smallint) is
  'Atomically adds or consumes one lesson credit for a coach-owned pack.';
