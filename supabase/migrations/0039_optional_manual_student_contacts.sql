alter table public.student_profiles
  drop constraint if exists student_profiles_phone_check;

alter table public.student_profiles
  add constraint student_profiles_phone_check check (
    trim(phone) = ''
    or char_length(trim(phone)) between 6 and 30
  );
...
drop index if exists public.uniq_student_profiles_normalized_phone;
...
comment on column public.student_profiles.phone is
  'Optional contact phone. An empty string represents no contact value.';

comment on column public.student_profiles.email is
  'Optional contact email. An empty string represents no contact value.';
